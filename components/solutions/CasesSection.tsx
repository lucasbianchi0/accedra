...
const casePhoto = (id: number) => `https://images.pexels.com/photos/${id}/pexels-photo-${id}.jpeg?auto=compress&cs=tinysrgb&w=1000`;
...
const imgFor = (industry: string) => CASE_IMG[industry] ?? casePhoto(577210);
    ...
    <section className="relative py-20 lg:py-28 overflow-hidden bg-gradient-to-b from-[#07101D] to-[#0A1424]">
      ...
      <div className="relative z-10 max-w-[1180px] mx-auto px-5 sm:px-8 lg:px-12">
        ...
        <div className="grid md:grid-cols-3 gap-5 lg:gap-6">
          {items.map((c, i) => (
            <motion.button key={i} type="button" onClick={() => setOpen(i)}
              initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.15 }}
              transition={{ duration: 0.5, delay: i * 0.1 }}
              className="group relative text-left rounded-2xl border overflow-hidden flex flex-col transition-all duration-300 hover:-translate-y-1.5 cursor-pointer"
              ...
              <div className="relative h-44">
                ...
                <img src={imgFor(c.industry)} alt="" aria-hidden="true"
      ...
      {/* Modal editorial reutilizado */}
      <CaseModal
        items={items.map((c) => ({ image: imgFor(c.industry), tag: c.industry, title: c.result, body: c.body ?? [c.challenge, c.solution] }))}
        open={open}
