...
export default function CaseDetail({ solucion, index }: { solucion: string; index: number }) {
  ...
  return (
    <main className="bg-[#07101D] min-h-screen relative">
      ...
      <div className="relative z-10 max-w-[1240px] mx-auto px-5 sm:px-8 lg:px-12 pt-20 lg:pt-24 pb-8">
        {/* Volver */}
        ...
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.1 }}
          ...
          <div className="relative rounded-panel p-px"
            ...
            {/* Alto relativo al viewport en vez de fijo: con 420px duros la portada se
      ...
      <div className="relative z-10 max-w-[1240px] mx-auto px-5 sm:px-8 lg:px-12 pb-16 lg:pb-24">
        <div className="grid lg:grid-cols-[1fr_340px] gap-8 lg:gap-14">
          ...
          <article>
            ...
            <div className="space-y-5">
              {item.body.map((p, k) => (
                <p key={k} className="text-gray-300 text-base leading-[1.85]">{p}</p>
              ))}
            </div>
          </article>
          ...
          <aside className="lg:sticky lg:top-24 h-fit">
            <div className="rounded-card border overflow-hidden"
              ...
              <div className="relative p-6 overflow-hidden"
                ...
                <p className="relative text-[11px] font-semibold uppercase tracking-[0.2em] text-blue-300 mb-2 flex items-center gap-1.5">
                  <TrendingUp size={12} /> Resumen
                </p>
                <p className="relative text-white text-base font-semibold leading-snug">{item.industry} · Caso de éxito</p>
              </div>
              ...
              <div className="divide-y divide-white/[0.07]">
                ...
                {!item.challenge && item.lead && <MetaRow icon={FileText} label="Descripción" value={item.lead} />}
              </div>

              {/* CTAs */}
              <div className="p-5 space-y-2.5 border-t border-white/[0.07]">
                <Link href="/#contacto"
                  className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl text-sm font-semibold text-white transition-all hover:gap-3"
                ...
                <Link href={sideCta.href}
                  className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl text-sm font-semibold text-gray-200 border border-white/10 hover:bg-white/5 transition-all">
                  {sideCta.label}
                </Link>
              </div>
            </div>
              ...
              {others.map((o) => (
                <Link key={o.i} href={`${basePath}/${o.i}`}
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <h3 className="text-white font-bold text-base leading-snug mb-2.5">{o.result}</h3>
                    <p className="text-gray-400 text-sm leading-relaxed mb-4 flex-1">{o.sub}</p>
  ...
  return (
    <div className="flex gap-3.5 p-5">
      ...
      <div className="min-w-0">
        <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-400 mb-1">{label}</p>
